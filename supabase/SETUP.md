...
2. **Create the tables** — in the Supabase dashboard open **SQL Editor → New
   ...
   **School 360** (no new migration — a read-only view over existing tables): see [`SCHOOL-360.md`](SCHOOL-360.md).
   ...
   | 33 | [`patch-30-legacy-account-sunset.sql`](patch-30-legacy-account-sunset.sql) | A server-visible ledger + workflow for retiring `hpf_users` staff accounts onto real Supabase Auth, detected one real `legacyLogin()` at a time — see [`LEGACY-SUNSET.md`](LEGACY-SUNSET.md) |
   | 34 | [`patch-31-learner-experience.sql`](patch-31-learner-experience.sql) | Fixes `submissions.learner_id` to reference `learners(id)` instead of `profiles(id)` — a learner never has a profiles row, so this FK was unsatisfiable for a real learner. Prerequisite for the real learner dashboard — see [`LEARNER-EXPERIENCE-SPEC.md`](LEARNER-EXPERIENCE-SPEC.md) |
   | 35 | [`patch-32-attendance-derivation.sql`](patch-32-attendance-derivation.sql) | Derives `school_returns.attendance_rate` from `attendance_records` instead of accepting a typed number, and records whether each figure is calculated, manually entered, or missing — see [`ATTENDANCE.md`](ATTENDANCE.md) |
   | 36 | [`patch-33-learner-identity.sql`](patch-33-learner-identity.sql) | Canonical learner identity: adds `learners.learner_code`, guarantees every enrollment/submission carries a stable `learner_id` by trigger, reconciles historical rows without guessing, and flags what it cannot resolve in `learner_identity_reviews` — ends name matching as an identity mechanism, see [`LEARNER-IDENTITY.md`](LEARNER-IDENTITY.md) |
   | 37 | [`patch-34-notifications-activation.sql`](patch-34-notifications-activation.sql) | Activates the existing `notifications` table: server-side producer (triggers + a sweep) for ten event types, priority/related-record/dedupe columns, and two RLS defects fixed — see [`NOTIFICATIONS.md`](NOTIFICATIONS.md) |
   | 38 | [`patch-35-offline-sync.sql`](patch-35-offline-sync.sql) | Server support for the offline-first PWA: a UNIQUE `field_reports.client_id` so a retried offline visit can never duplicate, plus the audit trigger `field_reports` never had — see [`OFFLINE-PWA.md`](OFFLINE-PWA.md) |
   | 39 | [`patch-36-field-report-me-link.sql`](patch-36-field-report-me-link.sql) | Lets "New field report" record a real M&E measurement inline: `field_reports.me_indicator_id`/`me_value`, derived into `me_indicator_values` by trigger — see [`FIELD-REPORT-FORMS.md`](FIELD-REPORT-FORMS.md) |
   ...
   **Learner 360** (no new migration — a read-only view over existing tables, sharing School 360's rendering helpers): see [`LEARNER-360.md`](LEARNER-360.md).
